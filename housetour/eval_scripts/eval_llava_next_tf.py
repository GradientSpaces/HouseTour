...
def generate_tf_answer(video_idx, question):
    max_frames_num = 96
    ...
    time_instruciton = f"The video lasts for {video_time:.2f} seconds, and {len(video[0])} frames are uniformly sampled from it. These frames are located at {frame_time}.Please answer the following question related to this video only with 'True' or 'False'."
...
if __name__ == "__main__":
    ...
    output_file = "eval_llava_next_tf.jsonl"
    ...
    for d in tqdm(tf):
        ...
        predicted_answer = generate_tf_answer(d["scene_id"], question)
        ...
        print(f"Rolling Accuracy: {correct_preds / (correct_preds + incorrect_preds)}")
