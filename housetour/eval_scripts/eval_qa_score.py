from autodq import eval_q_and_a
...
def evaluate_model(model, data_loader, cuda_enabled=False):
    ...
    with torch.no_grad():
        for samples in tqdm(data_loader):
            ...
            correctness_scores.append(int(autodq_eval['correctness']))
            print(f"Rolling Correctness Avg. : {sum(correctness_scores) / len(correctness_scores)}")
    ...
    return {"correctness": sum(correctness_scores) / len(correctness_scores)}
