...
def evaluate(model, val_loader, cuda_enabled, epoch):
    ...
    with torch.no_grad():
        for samples in tqdm(val_loader, desc="Evaluating"):
...
def train_epoch(
    ...
    eval_loss = evaluate(model, val_loader, cuda_enabled, epoch)

    checkpoint_path = os.path.join(cfg['run']['output_dir'], f"blip2_llama_e{epoch}_loss{eval_loss:.3f}.pth")
...
def train(cfg):
    ...
    if not eval_only:
        ...
        # Load the dataset
        full_dataset = HouseTour_Dataset(
            ...
            num_video_query_token=cfg["model"]["num_query_token"],
