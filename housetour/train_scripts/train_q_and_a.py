...
def evaluate(model, val_loader, cuda_enabled, step):
    model.eval()
    val_loss = 0.0
    with torch.no_grad():
...
def train_epoch(
    ...
    for batch_idx, samples in enumerate(tqdm(train_loader)):
        ...
        if (batch_idx + 1) % 5000 == 0:
            ...
            checkpoint_path = os.path.join(output_dir, f"train_qa_e{epoch}_iter{batch_idx + 1}_loss{eval_loss:.3f}.pth")
