...
def evaluate(model, val_loader, cuda_enabled, epoch):
    ...
    val_loss = 0.0
    with torch.no_grad():
        for samples in tqdm(val_loader, desc="Evaluating Captioning"):
...
def train_epoch(
    ...
    checkpoint_path = os.path.join(output_dir, f"train_capt_e{epoch}_loss{eval_loss:.3f}.pth")
