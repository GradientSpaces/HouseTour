...
from grandtour.models.blip2 import Blip2Base, disabled_train
from grandtour.models.modeling_llama import LlamaForCausalLM
# from grandtour.models.Qformer import BertEncoder
from transformers import LlamaTokenizer, BertConfig
# from transformers.models.bert.modeling_bert import BertEncoder
import einops
import copy
from grandtour.models.Qformer import BertConfig, BertLMHeadModel
...
class Blip2Llama(Blip2Base):
    def __init__(
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            vit_model, to_2tuple(img_size), drop_path_rate, use_grad_checkpoint, vit_precision
        )
        print(f"Visual Encoder No. of Param. : {sum(p.numel() for p in self.visual_encoder.parameters())}")
        print(f"Visual Encoder Image Size: {self.visual_encoder.image_size}")
        if freeze_vit:
        ...
        print(f"Q-Former No. of Param. : {sum(p.numel() for p in self.Qformer.parameters())}")
        ...
        if self.is_tf:
            self.TRUE_TOKEN_ID = self.llama_tokenizer.get_vocab()['▁True']
            self.FALSE_TOKEN_ID = self.llama_tokenizer.get_vocab()['▁False']
            print(f"Token IDs for True/False : {self.TRUE_TOKEN_ID}, {self.FALSE_TOKEN_ID}")
        else:
    ...
    def encode_Qformer_visual(self, image):
        ...
        with self.maybe_autocast():
            ...
            image_embeds = einops.rearrange(image_embeds, '(b t) q h -> b t q h', b=batch_size)
            image_embeds = einops.rearrange(image_embeds, 'b t q h -> b (t q) h')
            image_embeds = einops.rearrange(image_embeds, 'b (c mult) h -> b c mult h', c=int(time_length/self.context_size))
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(device)
            
            q_hidden_state = []
            for b in range(batch_size):
                b_image_embeds, b_image_atts = image_embeds[b], image_atts[b]
                query_tokens = self.query_tokens.expand(b_image_embeds.shape[0], -1, -1)
                query_output = self.Qformer.bert(
                    query_embeds=query_tokens,
                    encoder_hidden_states=b_image_embeds,
                    encoder_attention_mask=b_image_atts,
                    return_dict=True,
                )
                inter = query_output.last_hidden_state
                q_hidden_state.append(inter)
            q_hidden_state = torch.cat(q_hidden_state, dim=0)
            if len(q_hidden_state.size()) == 3:
                q_hidden_state = q_hidden_state.unsqueeze(0)
            inputs_llama = self.llama_proj(q_hidden_state)
            inputs_llama = einops.rearrange(inputs_llama, 'b c tok h -> b (c tok) h')
        ...
        if 'conv_type' in samples.keys() and samples['conv_type'] == 'multi':
            ...
            if isinstance(image, list):  # nb of frames of some videos is less than ${num_frm}
                ...
                for img in image:
                    ...
                    num_patch_tokens = self.num_query_token * math.ceil(img.shape[2] / self.context_size)
                    img_embeds, atts_img = self.encode_Qformer_visual(img)
                ...
                num_patch_tokens = self.num_query_token * math.ceil(image.shape[2] / self.context_size)
                img_embeds, atts_img = self.encode_Qformer_visual(image)
    ...
    def from_config(cls, cfg):
        ...
        if ckpt_path:
            ...
            # Skip the LoRA weights
            model_state_dict = {k: v for k, v in ckpt['model'].items() if "lora" not in k}
