...
from grandtour.common.utils import is_url
from grandtour.common.dist_utils import download_cached_file
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        qkv_bias = qkv_bias.to(self.qkv.weight.data.dtype)
...
#             self.head.bias.data.mul_(init_scale)
    ...
    def forward_features(self, x):
        ...
        # CLS token should match the precisions of the input
        cls_tokens = cls_tokens.to(x.dtype)
        x = torch.cat((cls_tokens, x), dim=1)
        if self.pos_embed is not None:
            # Pos embed should match the precisions of the input
            _pos_embed = self.pos_embed.to(x.dtype)
            x = x + _pos_embed
        x = self.pos_drop(x)
...
def interpolate_pos_embed(model, checkpoint_model):
    if 'pos_embed' in checkpoint_model:
        ...
        print(f"Number of extra tokens: {num_extra_tokens}")
        # height (== width) for the checkpoint position embedding
        orig_size = int((pos_embed_checkpoint.shape[-2] - num_extra_tokens) ** 0.5)
        # height (== width) for the new position embedding
        # I am changing this not to enforce quadratic images
        ...
        if orig_size != new_size:
            ...
            print(f"Positional Embed Shape: {checkpoint_model['pos_embed'].shape}")
