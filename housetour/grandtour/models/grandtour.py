...
class GrandTour(Blip2Base):
    ...
    def encode_videoQformer_visual(self, image):
        ...
        with self.maybe_autocast():
            ...
            # print(f"QFormer Output: {frame_hidden_state.shape}")
            # print(f"Frame Hidden State Shape: {frame_hidden_state.shape}")
            # frame attention
            if self.window_size <= 0:
                ...
                for i in range(0, time_length, self.stride):
                    ...
                    b_shape, tq_shape = clip_hidden_state.size()[0], clip_hidden_state.size()[1]
                    clip_atts = torch.ones(b_shape, tq_shape, dtype=torch.long)
                    clip_atts = clip_atts.to(device)
        ...
        if 'conv_type' in samples.keys() and samples['conv_type'] == 'multi':
            ...
            if isinstance(image, list):  # nb of frames of some videos is less than ${num_frm}
                ...
                for img in image:
                    ...
                    img_embeds, atts_img = self.encode_videoQformer_visual(img)
