...
from grandtour.models.base_model import all_gather_with_grad, concat_all_gather
from grandtour.models.blip2 import (
    Blip2Base,
    compute_sim_matrix,
    disabled_train,
)
from grandtour.models.blip2_outputs import BlipOutput, BlipOutputFeatures
...
class Blip2Qformer(Blip2Base):
    ...
    def __init__(
        ...
        if freeze_vit:
            ...
            n = 8  # Number of transformer blocks to unfreeze
            total_blocks = len(self.visual_encoder.blocks)

            # Freeze all layers
            for param in self.visual_encoder.patch_embed.parameters():
                param.requires_grad = False

            if self.visual_encoder.pos_embed is not None:
                self.visual_encoder.pos_embed.requires_grad = False

            if self.visual_encoder.cls_token is not None:
                self.visual_encoder.cls_token.requires_grad = False

            for blk_idx, blk in enumerate(self.visual_encoder.blocks):
                if blk_idx < total_blocks - n:  # Freeze earlier blocks
                    for param in blk.parameters():
                        param.requires_grad = False
                else:  # Unfreeze the last `n` blocks
                    for param in blk.parameters():
        ...
        self.conv_op = nn.Conv2d(
            ...
            out_channels=self.visual_encoder.num_features,   
            kernel_size=(5, 5),     
            stride=(4, 5),         
        )
    ...
    def forward(self, samples):
        ...
        # print(f"Text: {text}")
        # print(f"Image: {torch.isnan(image).any()}")
        # print(image.shape)
        device = image.device

        batch_size, n_images, channels, width, height = image.shape
        with self.maybe_autocast():
            image_concat = einops.rearrange(image, 'b n c w h -> (b n) c w h')
            img_encoded = self.visual_encoder(image_concat)
            image_embeds = self.ln_vision(img_encoded).to(device)
            image_embeds = einops.rearrange(image_embeds, '(b n) p h -> b (n p) h', b=batch_size)
        ...
        targets = torch.arange(0, bs, dtype=int).to(image.device)
